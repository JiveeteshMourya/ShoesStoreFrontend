...
export default function SellerOrdersPage() {
  ...
  const { items, total, page, totalPages, loading } = sellerOrders;
  const [status, setStatus] = useState("");

  const load = (p = 1, s = status) => {
    const params = { page: p, limit: 10 };
    if (s) params.status = s;
    dispatch(fetchSellerOrdersThunk(params));
  };
  ...
  return (
    <PageWrapper>
      <SectionTitle title="Orders" subtitle={`${total} total`} />
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3">
          {items.map(o => (
            <OrderCard key={o._id} order={o} linkBase="/seller/orders" />
          ))}
        </div>
      )}
