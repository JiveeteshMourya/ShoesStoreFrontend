...
export default function ProductCard({ product }) {
  ...
  return (
    <Link to={href} className="group block">
      <div className="overflow-hidden rounded-(--radius-md) bg-(--color-surface) aspect-square">
        {imageUrl ? (
          <img
            src={imageUrl}
            alt={name}
      ...
      <div className="mt-3 space-y-0.5">
        <p className="text-xs text-(--color-secondary) uppercase tracking-wide">{brand}</p>
        <p className="text-sm font-medium text-(--color-primary) leading-snug line-clamp-2">
          {name}
        </p>
        <div className="flex items-center justify-between pt-1">
          <p className="text-sm font-semibold text-(--color-primary)">{formatPrice(basePrice)}</p>
          {reviewCount > 0 && (
